<!-- Διεπαφή για τηΝ προβολή των εργασιών που περιέχουν λέξεις κλειδιά που εμφανίζονται στο σύννεφο λέξεων των μαθητών-->
...
<div class="row">
	<div class="col-sm-8 col-md-8 col-lg-8">
		...
		$query2 = 'SELECT erg_erg_id FROM ergasia_has_keyword WHERE keyw_keyw_id="'.$keyw.'" AND erg_erg_id IN (SELECT erg_id FROM ergasia WHERE erg_is_visible=1 AND les_les_id IN (SELECT par_les_id FROM parakolouthei WHERE math_math_id="'.$mathId.'"))';
		$result2 = $connection->query($query2);
		$row2 = $result2->fetch_array();
		$nr=$result2->num_rows; //αριθμός γραμμών που επέστρεψε το ερώτημα
		...
		if ($pn == 1) {
		    ...
		    $centerPages .= '&nbsp; <a href="' . 'index.php?content=ergasies_provoli_cloud_mathiti&kwd='.$keyw.'&word='.$_GET['word'].'&pn='. $add1 . '">' . $add1 . '</a> &nbsp;';
		} else if ($pn == $lastPage) {
		    $centerPages .= '&nbsp; <a href="' . 'index.php?content=ergasies_provoli_cloud_mathiti&kwd='.$keyw.'&word='.$_GET['word'].'&pn=' . $sub1 . '">' . $sub1 . '</a> &nbsp;';
		    $centerPages .= '&nbsp; <span class="pagNumActive">' . $pn . '</span> &nbsp;';
		} else if ($pn > 2 && $pn < ($lastPage - 1)) {
		    $centerPages .= '&nbsp; <a href="' . 'index.php?content=ergasies_provoli_cloud_mathiti&kwd='.$keyw.'&word='.$_GET['word'].'&pn=' . $sub2 . '">' . $sub2 . '</a> &nbsp;';
		    $centerPages .= '&nbsp; <a href="' . 'index.php?content=ergasies_provoli_cloud_mathiti&kwd='.$keyw.'&word='.$_GET['word'].'&pn=' . $sub1 . '">' . $sub1 . '</a> &nbsp;';
		    $centerPages .= '&nbsp; <span class="pagNumActive">' . $pn . '</span> &nbsp;';
		    $centerPages .= '&nbsp; <a href="' . 'index.php?content=ergasies_provoli_cloud_mathiti&kwd='.$keyw.'&word='.$_GET['word'].'&pn=' . $add1 . '">' . $add1 . '</a> &nbsp;';
		    $centerPages .= '&nbsp; <a href="' . 'index.php?content=ergasies_provoli_cloud_mathiti&kwd='.$keyw.'&word='.$_GET['word'].'&pn=' . $add2 . '">' . $add2 . '</a> &nbsp;';
		...
		WHERE erg_is_visible=1 AND erg_id IN (SELECT erg_erg_id FROM ergasia_has_keyword WHERE keyw_keyw_id= '$keyw') ORDER BY erg_datetime DESC $limit";
    	...
    	if ($pn != 1) {
        $previous = $pn - 1;
        $paginationDisplay .=  '&nbsp;  <a href="' . 'index.php?content=ergasies_provoli_cloud_mathiti&kwd='.$keyw.'&word='.$_GET['word'].'&pn=' . $previous . '"> Προηγούμενη</a> ';
    	} 
		    ...
		    if ($pn != $lastPage) {
		        $nextPage = $pn + 1;
		        $paginationDisplay .=  '&nbsp;  <a href="' . 'index.php?content=ergasies_provoli_cloud_mathiti&kwd='.$keyw.'&word='.$_GET['word'].'&pn=' . $nextPage . '"> Επόμενη</a> ';
		    } 
		}

		if (!$row2){ //έλεγχος αν έχουν καταχωρυθεί εργασίες
